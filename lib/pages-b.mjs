...
export const shopPage = (store, ctx) => {
  ...
  return {
    ...
    desc: 'Every device in the ' + store.brand + ' store. Free nationwide shipping, pay by bank transfer or Paystack.',
    path: '/shop/',
    active: 'shop',
    body: body,
    jsonld: []
...
export const checkoutPage = (store, ctx) => {
  ...
  return {
    ...
    desc: 'Place your order with ' + store.brand + '. Free nationwide shipping, pay by bank transfer or Paystack.',
    path: '/checkout/',
    active: 'checkout',
    body: body,
    jsonld: []
...
export const policyPages = (store) => [
  {
    ...
    body: prose('Delivery and returns',
      ...
      ['How long it takes', [store.delivery, 'We call you before dispatch to agree a delivery window.']],
      ['What it costs', ['Delivery is free. There is no separate shipping charge on any order.']],
      ['Paying', ['Payment is by bank transfer or Paystack, arranged when we confirm your order. Nothing is charged on this site.']],
  ...
  },
  {
    ...
    body: prose('Payment',
      ...
      ['How it works', ['Send your order from the store, or message us on WhatsApp.', 'We confirm the price, the colour and the delivery window with you.', 'You pay by bank transfer or by Paystack, and the device ships once it clears.']],
      ['Bank transfer or Paystack', ['Ask us for the account details, or for a Paystack link, when we confirm your order.', 'We only ever send payment details from our own WhatsApp number or over the phone.']],
      ['Paying by card', ['Ask for a Paystack link when we confirm your order, and pay with a card.']],
  ...
  const body = crumb(store, [['Contact']]) +
    ...
    '<section class="sect-tight"><div class="wrap">' +
      '<div class="grid grid-2">' +
        ...
        '<div class="card panel">' +
          '<h3>Visit us</h3>' +
          '<p class="muted">Come to the shop to see a device before you buy, or to collect an order.</p>' +
          '<p class="tel-lg">' + esc(store.address) + '</p>' +
          '<a class="btn btn-ghost" href="https://maps.google.com/?q=' + encodeURIComponent(store.address) + '" target="_blank" rel="noopener">' + svgIc(IC.pin, 'ic') + 'Open in Maps</a>' +
        '</div>' : '') +
      '</div>' +
